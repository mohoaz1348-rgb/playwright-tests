...
Это проект, который демонстрирует использование python, playwright, pytest, allure, GitHub Actions для написания и запуска UI-тестов. При использовании GitHub Actions происходит подготовка окружения с помощью uv, запуск тестов, генерация отчета Allure (на основе истории предыдущих прогонов и файла executor.json) и выкладывание итогового отчета на gh-pages данного репозитория. Полученный отчет доступен по ссылке - https://mohoaz1348-rgb.github.io/playwright-tests/index.html.
Данный проект не служит примером написания каких-то сложных тестов на playwright. Скорее он служит как пример построения удобной инфраструктуры для их написания, отладки и запуска.
...
После клонирования репозитория и установки uv, выполните команды:

`uv python install`    # Версия питона, подтягивается из файла .python-virsion     
...
См. [файл](./.github/workflows/test.yml)
